...
export const mdxComponents = {
    ...
    img: ({ src, alt, ...props }: ComponentProps<'img'>) => (
        <span className="block my-10 rounded-md overflow-hidden border border-foreground/10 bg-foreground/5">
    ...
    pre: ({ children, ...props }: ComponentProps<'pre'>) => (
        <pre className="relative my-8 overflow-x-auto p-6 rounded-md bg-foreground/5 border border-foreground/10 text-sm font-mono leading-relaxed" {...props}>
            {children}
        </pre>
    ),
};